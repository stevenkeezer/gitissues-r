  ...
  return (
    <div>
      <h1 className="total-results-text">
        🛸 Showing Repos {props.totalSearchResult} pages🛸
      </h1>
      <Pagination>
        <Pagination.First onClick={() => handleOnChange(1)} />
        <Pagination.Prev
          onClick={() => {
            if (props.currentPage !== 1) handleOnChange(props.currentPage - 1);
          }}
        />
        {items}
        <Pagination.Next
          onClick={() => {
            if (props.currentPage !== props.totalSearchResult)
              handleOnChange(props.currentPage + 1);
          }}
        />
        <Pagination.Last
          onClick={() => handleOnChange(props.totalSearchResult - 1)}
        />
      </Pagination>
